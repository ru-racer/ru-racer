function [T,z,u,out,Xd]=restrict_stanf_Uturn(vd,tf,ru)
% close all;
Del_T=0.002;
T=0:Del_T:tf;
...
if rem(i,PH)==1
    tic        
      u(:,i+1)=NMPC(Xd(:,i+10),z(:,i),u(:,i),Del_T,out(:,i),1,.4,max_time);
...
%    figure
%    plot(out(3,10:end),out(end,10:end),'b') %stablity bound
% %     figure
% % % %  hold on
...
%   u(2)=PID_vel(z(:,i),u,Xd(:,i));
    ...
    z(:,i+1)=z(:,i)+ dz*DelT;
    x=z(:,i+1);
    J=J+1*sqrt((xd(1:2)-x(1:2))'*((xd(1:2)-x(1:2)) ) + .0*( (xd(4:6)-x(4:6))'*(xd(4:6)-x(4:6)) ) + 0.0*(u'*u) );
    dz0=dz;
...
J=1*J+1*stability_bound_cost(x,out(:,end),0);
