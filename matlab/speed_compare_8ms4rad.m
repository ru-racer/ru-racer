...
tf=2.5;
ru=4;
% [Tst,zst,ust,outst,Xdst]=restrict_stanf_Uturn(vd,tf,ru); %1.1 .4
% [T,z,u,out,Xd]=Uturn(vd,tf,ru); %1.1 .4
...
if (z(1,i)<0 && z(2,i)<0 && Finished==0)
    Finished=T(i)
    string = sprintf('T_{f P}= %.2f (s)',T(i));
    text(z(1,i)+.2,z(2,i),string,'Parent',ax_path,'fontsize',14)
    plot(Xd(1,i),Xd(2,i),'Parent',ax_path,'Marker','o','Markersize',6,'Color',[1 0 0],'Linewidth',3);
end
 if (zst(1,i)<0 && zst(2,i)<0 && FinishedSt==0)
    FinishedSt=Tst(i)
    string = sprintf('T_{f RB}= %.2f (s)',Tst(i));
    text(zst(1,i)+.2,zst(2,i)-.5,string,'Parent',ax_path,'fontsize',14,'Color',[.5 .5 .5])
 end
