...
vd=7;
tf=3;
% [Tst,zst,ust,outst,Xdst]=restrict_stanf_Uturn(vd,tf);
% [T,z,u,out,Xd]=Uturn(vd,tf);
 % hold on
F_S=14;
...
plot3(T(10:end),out(end,10:end)/1.8,z(6,10:end)/3.5,'Parent',ax_phase,'LineWidth',2,'DisplayName','Proposed','Color',[0 0 0],'LineStyle','-');
hold on
plot3(Tst(10:end),outst(end,10:end)/2,zst(6,10:end)/4,'Parent',ax_phase,'LineWidth',2,'DisplayName','Restrict Boundary','Color',[.4 .4 .4],'LineStyle','-.');
legend(ax_phase,'show'); % title({'path tracking'}); 
hold on
for i=10:10:length(T)
if stability_bound_cost(out(end,i)/1.8,z(6,i)/3.5,0)>0
    plot3(T(i),out(end,i)/1.8,z(6,i)/3.5,'*r');
end
...
% ylim(axes1,[min(x)-.5 max(x)+2.5]);xlim(axes1,[min(y)-1 max(y)+2]);
xlabel('t','FontSize',F_S); ylabel({'rad'},'FontSize',F_S);
hold on
...
plot(T,ust(1,1:end)/1.3,'Parent',ax_str,'LineWidth',2,'DisplayName','Restrict Boundary','Color',[.4 .4 .4],'LineStyle','-.');
