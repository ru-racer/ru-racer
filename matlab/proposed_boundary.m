...
z(:,1)=[Xd(1,1) -0.05 .2 .99*v0 .2*v0 0.2 .99*v0*ones(1,4)/.055]';
% R=40; wr=.25;
...
[out(:,i+1),dz]=wheel_model1(z(:,i),u(:,i));
z(:,i+1)=z(:,i)+ dz*Del_T;
DELTA(i)=u(1,i);
end
% hold on
...
% %     hold off;
     robot(z(:,i),DELTA(1,i),max(z(1,:))/30,max(z(1,:))/25,'-',[0 0 0],out(:,i));
...
F_z=.5*Par.M(1,1)*[Par.g*Par.Car.L_r/Par.Car.L-Par.Car.h*(0*(dotzo(4)/Par.Car.L)+(0*dotzo(5)/Par.Car.b))
                   Par.g*Par.Car.L_r/Par.Car.L-Par.Car.h*(0*(dotzo(4)/Par.Car.L)-(0*dotzo(5)/Par.Car.b))
                   Par.g*Par.Car.L_f/Par.Car.L-Par.Car.h*(0*(dotzo(4)/Par.Car.L)-(0*dotzo(5)/Par.Car.b))
                   Par.g*Par.Car.L_f/Par.Car.L-Par.Car.h*(0*(dotzo(4)/Par.Car.L)+(0*dotzo(5)/Par.Car.b))];
...
J=1*J+.1*stability_bound_cost(x,other,0);
