...
%   delta_near =1;
    ...
    this.Model_Size=6;
    this.Input_Size=2;  
    this.model = Bike([0;.01;pi/2;3;0;0],[0;0],map);
    this.goal = [0;-.05;2*pi+pi/2;1;0;0];%

%   this.model = Bike([0;1;pi/2;1;0;0],[0;0],map);
...
%   this.XY_BOUNDARY = [[-2.6;-2;-2*pi;.5;-8;-25] [.3;3.5;2*pi;5;3;25]];
    this.XY_BOUNDARY = [[-2.6;-.8;-2*pi;.5;-.5;-5] [.3;3.5;2.5*pi;3.5;.5;5]];
...
%      this.model_history{this.nodes_added}= this.model;
       ...
       this.distance(this.nodes_added) = norm([this.goal(1:2)-New_Node(1:2);.2*(this.goal(4)-New_Node(4));...
           .2*this.solvePI(this.goal(3)-New_Node(3)); ...
           0*abs(atan2(this.goal(2)-New_Node(2),this.goal(1)-New_Node(1)))  ],2);
...
%              plot(this.model_history{i}.All_States(1,:),this.model_history{i}.All_States(2,:),'r')
             plot([this.tree(1,current_index);this.tree(1, this.parent(current_index))], ...
                 [this.tree(2, current_index);this.tree(2, this.parent(current_index))],'-','LineWidth', 0.2,'Color',[0 1 0]);
             plot([this.tree(1,current_index);this.tree(1, this.parent(current_index))], ...
                 [this.tree(2, current_index);this.tree(2, this.parent(current_index))],'.','Color',1*[0 1 1]);
             drawn_nodes(current_index) = true;
         end
         current_index = this.parent(current_index); 
         end
    end
    this.plot_circle(this.goal(1),this.goal(2),this.delta_goal);
...
%     for i =1:length(this.goal_reached)
     if ~isempty(this.goal_reached)
        ...
        if this.cumcost(this.goal_reached(i))==min(this.cumcost(this.goal_reached))
            ...
            plot([this.tree(1,Cind) this.tree(1,Pind)],[this.tree(2,Cind) this.tree(2,Pind)],'b','LineWidth',1)
%             this.plot_circle_s(this.tree(1,Cind),this.tree(2,Cind),this.tree(4,Cind)/30,'b')
            Plotcar(this.tree(:,Cind),this.tree_input(1,Cind),.22,.16,'-',[0 0 0]);
        else
            plot([this.tree(1,Cind) this.tree(1,Pind)],[this.tree(2,Cind) this.tree(2,Pind)],'r','LineWidth',1)
        end
        if Pind == 1
            break
        end
